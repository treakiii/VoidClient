...
public class RenderUtil implements Util {
    ...
    public static void drawGradientRect(MatrixStack matrix, float x, float y, float w, float h, int startColor,
            int endColor) {
        float f = (float) (startColor >> 24 & 255) / 255.0F;
        float g = (float) (startColor >> 16 & 255) / 255.0F;
        float h1 = (float) (startColor >> 8 & 255) / 255.0F;
        float j = (float) (startColor & 255) / 255.0F;
        float f1 = (float) (endColor >> 24 & 255) / 255.0F;
        float g1 = (float) (endColor >> 16 & 255) / 255.0F;
        float h2 = (float) (endColor >> 8 & 255) / 255.0F;
        float j1 = (float) (endColor & 255) / 255.0F;

        BufferBuilder bufferBuilder = Tessellator.getInstance()
                .begin(VertexFormat.DrawMode.QUADS, VertexFormats.POSITION_COLOR);
        bufferBuilder.vertex(matrix.peek().getPositionMatrix(), x + w, y, 0.0F).color(g, h1, j, f);
        bufferBuilder.vertex(matrix.peek().getPositionMatrix(), x, y, 0.0F).color(g, h1, j, f);
        bufferBuilder.vertex(matrix.peek().getPositionMatrix(), x, y + h, 0.0F).color(g1, h2, j1, f1);
        bufferBuilder.vertex(matrix.peek().getPositionMatrix(), x + w, y + h, 0.0F).color(g1, h2, j1, f1);
    ...
    public static MatrixStack matrixFrom(Vec3d pos) {
        ...
        matrices.translate(pos.getX() - camera.getPos().x, pos.getY() - camera.getPos().y,
