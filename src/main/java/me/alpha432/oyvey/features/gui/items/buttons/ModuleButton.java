...
public class ModuleButton
        ...
        if (!this.module.getSettings().isEmpty()) {
            for (Setting<?> setting : this.module.getSettings()) {
                ...
                if ((setting.getValue() instanceof String || setting.getValue() instanceof Character)
                        && !setting.getName().equalsIgnoreCase("displayName")) {
    ...
    public void drawScreen(DrawContext context, int mouseX, int mouseY, float partialTicks) {
        ...
        this.onUpdateHover(mouseX, mouseY);
    ...
    public void drawTooltip(DrawContext context, int mouseX, int mouseY) {
        if (this.shouldShowTooltip()) {
            ...
            int width = mc.textRenderer.getWidth(desc) + 4;
            float xPos = mouseX + 10;
            float yPos = mouseY - 10;
            RenderUtil.rect(context.getMatrices(), xPos, yPos, xPos + width, yPos + 12, 0xAA000000);
            Color rainbow = ColorUtil.rainbow(0);
            RenderUtil.rect(context.getMatrices(), xPos, yPos, xPos + width, yPos + 0.5f, rainbow.getRGB());
            context.drawTextWithShadow(mc.textRenderer, desc, (int) xPos + 2, (int) yPos + 2, -1);
        }
    }
